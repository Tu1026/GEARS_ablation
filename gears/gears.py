...
# from GSP.evaluation import ihab_et_al_benchmark_gears
...
class GEARS:
    ...
    def __init__(
        ...
        if self.weight_bias_track:
            ...
            wandb.init(project=proj_name, name=exp_name, tags=tags)
            self.wandb = wandb
    ...
    def model_initialize(
        ...
        no_perturb=False,
        no_coexpression=False,
        no_perturb_graph=False,
        gnn='SGConv',
        **kwargs
    ):
        """
        ...
        self.config = {
            ...
            "no_coexpression": no_coexpression,
            "no_perturb_graph": no_perturb_graph,
            "ablation_mode": ablation_mode,
            "gene_indicies": torch.arange(self.num_genes),
            "pert_indicies": torch.arange(self.num_perts),
        ...
        #####################-----------------Ablation Mode-----------------#####################
        if self.config["ablation_mode"] == "dummy-gene-graph":
            self.config["G_coexpress"] = torch.stack([torch.arange(len(self.gene_list)), torch.arange(len(self.gene_list))])
            self.config["G_coexpress_weight"] = torch.Tensor([1]).repeat(len(self.gene_list))
            
        elif self.config["ablation_mode"] == "dummy-pert-graph":
            self.config["G_go"] = torch.stack([torch.arange(len(self.pert_list)), torch.arange(len(self.pert_list))])
            self.config["G_go_weight"] = torch.Tensor([1]).repeat(len(self.pert_list))
        elif self.config["ablation_mode"] == "dummy-both-graph":
            self.config["G_coexpress"] = torch.stack([torch.arange(len(self.gene_list)), torch.arange(len(self.gene_list))])
            self.config["G_coexpress_weight"] = torch.Tensor([1]).repeat(len(self.gene_list))
            self.config["G_go"] = torch.stack([torch.arange(len(self.pert_list)), torch.arange(len(self.pert_list))])
            self.config["G_go_weight"] = torch.Tensor([1]).repeat(len(self.pert_list))
        elif self.config["ablation_mode"] is False:
            pass
        
        elif self.config["ablation_mode"] == "scrambled-gene-emb":
            self.config["gene_indicies"] = torch.randperm(self.num_genes)
            self.config["G_coexpress"] = self.config["gene_indicies"][self.config["G_coexpress"]]
        elif self.config["ablation_mode"] == "scrambled-pert-emb":
            self.config["pert_indicies"] = torch.randperm(self.num_perts)
            self.config["G_go"] = self.config["pert_indicies"][self.config["G_go"]]
        elif self.config["ablation_mode"] == "scrambled-both-emb":
            self.config["gene_indicies"] = torch.randperm(self.num_genes)
            self.config["pert_indicies"] = torch.randperm(self.num_perts)
            self.config["G_coexpress"] = self.config["gene_indicies"][self.config["G_coexpress"]]
            self.config["G_go"] = self.config["pert_indicies"][self.config["G_go"]]
            ...
            raise ValueError("ablation_mode not recognized")
        #####################-----------------Ablation Mode-----------------#####################
    ...
    def save_model(self, path):
        ...
        base_path = "models"
        if self.wandb:
            path = os.path.join(base_path, self.wandb.run.id)
        ...
        if self.wandb:
            self.wandb.save(f'{path}/*', base_path=base_path)
        ...
        for pert in pert_list:
            for i in pert:
                if i not in self.pert_list:
                    raise ValueError(
                        i + " is not in the perturbation graph. "
    ...
    def plot_perturbation(self, query, save_file=None):
        ...
        truth = truth.toarray() if issparse(truth) else truth
    ...
    def train(self, epochs=20, lr=1e-3, weight_decay=5e-4):
        ...
        print_sys(log.format(0, 0, 0))

        # lst = []
        # list(map(lambda x: lst.append(x.pert_idx) , list(self.dataloader["train_loader"])))
        # print()
        # pert_embeddings = self.model.pert_emb(torch.LongTensor(list(range(self.model.num_perts))).to(self.model.args['device']))
        # pert_ad = sc.AnnData(pert_embeddings.to('cpu').detach().numpy(), obs= self.pert_list)
        ...
        # sns.scatterplot(data=plot_df_pert, x='x', y='y', ax=ax)
        # # Log the plot using wandb
        # self.wandb.log({"Pert embedding over time": fig}) 
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                if step % 50 == 0:
                    ...
                    # list(map(lambda x: lst.append(x.pert_idx) , list(self.dataloader["train_loader"])))
                    # print()
                    # pert_embeddings = self.model.pert_global_emb.reshape( self.num_perts, -1)
                    # pert_ad = sc.AnnData(pert_embeddings.to('cpu').detach().numpy(), obs= self.pert_list)
                    # sc.pp.neighbors(pert_ad, use_rep='X', n_neighbors=30, metric='cosine')
                    # sc.tl.umap(pert_ad)
                    # plot_df_pert = pd.DataFrame(pert_ad.obsm['X_umap'])
            ...
            # if self.wandb:
            #     # self.wandb.log_model(self.model, step=epoch+1)
            #     self.wandb.log_model(os.path.join(path, "model.pt"), "large_gears", aliases=[f"epoch-{epoch+1})"])
            # print(f"{epoch+1} epoch model saved at {path}!!!!")
        ...
        # ihab_et_al_benchmark_gears(self.adata,
        #             self.dataloader.values(), self.dataset_name, self.best_model, self.device)
    ...
    def resume_test(self, epochs=20, lr=1e-3, weight_decay=5e-4):
        """
        Train the model
